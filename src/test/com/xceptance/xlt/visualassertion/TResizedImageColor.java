...
/**
 * If one image is smaller then the other, the resizeImage method will adapt their size and fill the formerly
 * nonexistent pixels with transparent black. This test tests if the previously nonexistent parts in the reference image
...
public class TResizedImageColor
{
    ...
    /**
     * Tests if two black images of different sizes are recognized as different despite colTolerance significantly above
     * 1. They should be.
     * 
     * @throws IOException
     */
    ...
    {
        final ImageComparison imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 10, 10, 0.01, false, false, 3, 3, false);
        final boolean result = imagecomparison.isEqual(reference, screenshot, fileMask, fileOut, differenceFile);
        Assert.assertFalse("Images of different size shoudn't be equal", result);
    }

    /**
     * Tests if two black images of different sizes are recognized as different with a colTolerance value of 0.
     * 
     * @throws IOException
     */
    @Test
    ...
    {
        final ImageComparison imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 1, 0.00, 0.01, false, false, 3, 3, false);
        final boolean result = imagecomparison.isEqual(reference, screenshot, fileMask, fileOut, differenceFile);
        Assert.assertFalse("Images of different size shoudn't be equal", result);
    }
