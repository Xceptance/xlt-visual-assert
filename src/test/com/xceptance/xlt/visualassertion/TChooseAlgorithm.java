...
/**
 * Tests if the specified algorithm is chosen and if an exception is thrown in case the parameter for algorithm does not
 * match any existing algorithm. Like in case of typos. The ImageComparison object is always initialized with the same
 * parameters for pixelPerBlockXY, threshold and trainingMode, only the parameter for the alorithm differs.
 * 
 * @author daniel
 */
public class TChooseAlgorithm
{
    ...
    private ImageComparison exactComp = new ImageComparison(Algorithm.MATCH, 10, 10, 50, 0.2, 0.01, false, false, 3, 3, false);

    private final ImageComparison pixelFuzzyComp = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 50, 0.2, 0.01, false, false, 3, 3,
                                                                       false);

    private final ImageComparison fuzzyComp = new ImageComparison(Algorithm.FUZZY, 10, 10, 50, 0.2, 0.01, false, false, 3, 3, false);
    ...
    {
        initializeImages();
        exactComp = new ImageComparison(Algorithm.MATCH, 10, 10, 20, 0.2, 10, false, false, 3, 3, false);
