 ...
 * if these areas in the mask image are black and if a difference in the images is detected if these areas in the mask
 * image are white. <br>
 * 
 * @author damian
 */
public class TMask
{
    ...
    private final static int rgbTransparentWhite = new Color(255, 255, 255, 0).getRGB();
    ...
    public static void initializeImages() throws IOException
    {
        ...
        final BufferedImage maskImage = new BufferedImage(300, 300, BufferedImage.TYPE_INT_ARGB);
        for (int w = 0; w < screenshot.getWidth(); w++)
        {
            for (int h = 0; h < screenshot.getHeight(); h++)
            {
    ...
    /**
     * Tests if the parts where the mask is transparent and there were differences was marked
     * 
     * @throws IOException
     */
    @Test
    ...
    {
        final ImageComparison imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 10, 0.1, 0.01, false, false, 3, 3, false);

        final boolean result = imagecomparison.isEqual(reference, screenshot, fileMask, fileOut, differenceFile);
        Assert.assertFalse("A difference that wasn't masked went undetected", result);
    ...
    /**
     * Tests if the parts where the mask is black and there were differences was marked
     * 
     * @throws IOException
     */
    @Test
    ...
    {
        final ImageComparison imagecomparison = new ImageComparison(Algorithm.FUZZY, 10, 10, 10, 0.1, 0.01, false, false, 3, 3, false);
