...
/**
 * Tests the return value of ImageComparison for differently sized images. It should return false for images of
 * different sizes.
 * <p>
 * Also tests if the correct parts are marked in the correct color. <br>
 * And if a difference in size will return false if the training mode is on. It should.
 * <p>
...
public class TMarkSizes
{
    ...
    ImageComparison imgCompare = new ImageComparison(Algorithm.FUZZY, 10, 10, 2, 0.00, 0.01, false, false, 3, 3, true);

    ImageComparison imgCompareTrain = new ImageComparison(Algorithm.FUZZY, 10, 10, 2, 0.00, 0.01, true, false, 3, 3, true);
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, slim, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, wide, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, low, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, high, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, slimAndLow, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, wideAndHigh, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertFalse(imgCompare.isEqual(reference, switched, maskFile, outPutfile, differenceFile));
    }
    ...
    {
        Assert.assertTrue(imgCompareTrain.isEqual(reference, switched, maskFile, outPutfile, differenceFile));
    }

    /**
     * Width and height of the pictures are switched. Check if the previously not existent areas are marked. Checks
     * again after it was done the other way round.
     * 
     * @throws IOException
     */
    @Test
    public void checkMarkedAreaBroadFormat() throws IOException
    {
        ...
        for (int w = 0; w < maxWidth; w++)
        {
            for (int h = 0; h < maxHeight; h++)
            {
                ...
                {
                    Assert.assertEquals(Color.BLACK.getRGB(), marked.getRGB(w, h));
                }
                else
                {
                    Assert.assertEquals(transparentBlack.getRGB(), marked.getRGB(w, h));
                }
            }
        }
    ...
    public void checkMaskAreaBroadFormat() throws IOException
    {
        ...
        paintWhite(switched);
        imgCompareTrain.isEqual(reference, switched, maskFile, outPutfile, differenceFile);
        ...
        for (int w = minWidth; w < maxWidth; w++)
        {
            for (int h = minHeight; h < maxHeight; h++)
            {
                Assert.assertEquals(transparentBlack.getRGB(), mask.getRGB(w, h));
            }
        }
    ...
    /**
     * Width and height of the images are equal. Checks that it returns true and more importantly, that nothing was,
     * masked or changed in the difference image.
     * 
     * @throws IOException
     */
    @Test
    public void checkMarkedAreaSameSize() throws IOException
    {
        ...
        final BufferedImage imgSameSize = new BufferedImage(reference.getWidth(), reference.getHeight(), BufferedImage.TYPE_INT_ARGB);
        ...
        for (int w = 0; w < reference.getWidth(); w++)
        {
            ...
            {
                Assert.assertEquals(Color.BLACK.getRGB(), reference.getRGB(w, h));
            }
        }
        for (int w = 0; w < imgSameSize.getWidth(); w++)
        {
            ...
            {
                Assert.assertEquals(Color.BLACK.getRGB(), imgSameSize.getRGB(w, h));
            }
        }

        Assert.assertTrue(imgCompare.isEqual(reference, imgSameSize, maskFile, outPutfile, differenceFile));

        // Check if a difference file was made. It shoudn't have been
        Assert.assertFalse(differenceFile.exists());

        imgCompareTrain.isEqual(reference, imgSameSize, maskFile, outPutfile, differenceFile);
        ...
        for (int w = 0; w < mask.getWidth(); w++)
        {
            ...
            {
                Assert.assertEquals(transparentWhite.getRGB(), mask.getRGB(w, h));
            }
        }
    }
