...
/**
 * Tests if differences are marked correctly. Tests if differences in a white image were marked red and if differences
 * in a red image were marked green. Also tests the shape of the marking. Also tests if the markingX and the markingY
 * parameters are working as intended. Since the marking is done outside of the comparison algorithm, it only tests
 * pixelFuzzyEqual.
 * 
 * @author damian
 */
public class TMarks
{
    ...
    /**
     * Tests if a difference, a white rectangle where there was none in the reference image was marked red. Only tests
     * if they were marked red, not how they were marked, ie if it was a rectangle of the correct size.
     * 
     * @throws IOException
     */
    @Test
    ...
    {
        final ImageComparison imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 1, 0.1, 0.01, false, false, 3, 3, false);
    ...
    /**
     * Tests if a difference, a red rectangle where there was none in the reference image was marked green. Only tests
     * if they were marked green, not how they were marked, ie if it was a rectangle of the correct size.
     * 
     * @throws IOException
     */
    @Test
    public void correctlyMarkedGreen() throws IOException
    {
        ...
        Assert.assertTrue("The difference wasn't marked green - " + "correctlyMarkedGreen", hasGreen);
    }

    /**
     * Tests the markingX and the markingY parameters. Tests if parameters of 10, 10 result in a 10 * 10 square.
     * 
     * @throws IOException
     */
    @Test
    public void correctShapeTenTen() throws IOException
    {
        ...
        final ImageComparison imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, markingX, markingY, 1, 0.1, 0.01, false, false, 3,
                                                                    3, false);
        imagecomparison.isEqual(reference, screenshot, fileMask, fileOut, differenceFile);
        final BufferedImage output = ImageIO.read(fileOut);
        ...
        // are saved
        final BufferedImage marking = new BufferedImage(output.getWidth(), output.getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    /**
     * Tests the markingX and the markingY parameter. Tests if parameters of 5, 10 result in a rectangle with a height
     * of five pixels and a width of ten.
     * 
     * @throws IOException
     */
    @Test
    ...
    /**
     * Tests the markingX and the markingY parameter. Checks if everything gets marked with parameters of 1, 1, ie
     * rectangles with a width and height of 1. Also tests if exactly the same happens if only one of them is 1, because
     * it would make no sense to draw a rectangle with a width or height of one. This DOES NOT WORK at the moment. Areas
     * that should be marked red are marked green, areas that should be marked green are marked red.
     * 
     * @throws IOException
     */
    @Test
    public void correctShapeOneOne() throws IOException
    {
        ...
        ImageComparison imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, markingX, markingY, 1, 0.1, 0.01, false, false, 3, 3,
                                                              false);
        imagecomparison.isEqual(reference, screenshot, fileMask, fileOut, differenceFile);
        ...
        imagecomparison = new ImageComparison(Algorithm.COLORFUZZY, markingX, markingY, 1, 0.1, 0.01, false, false, 3, 3, false);
        imagecomparison.isEqual(reference, screenshot, fileMask, fileOut, differenceFile);
