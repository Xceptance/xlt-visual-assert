...
/**
 * Tests if small non-marked gaps in the maskImage are closed. Also tests if the maskImage stays the same otherwise.
 * 
 * @author daniel
 */
public class TCloseMask
{
    ...
    private final ImageComparison train = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 10, 0.0, 0.01, true, true, 3, 3, false);

    private final ImageComparison compare = new ImageComparison(Algorithm.COLORFUZZY, 10, 10, 10, 0.0, 0.01, false, false, 3, 3, false);
    ...
    {
        final BufferedImage maskImage = new BufferedImage(200, 200, BufferedImage.TYPE_INT_ARGB);
        for (int w = 0; w < reference.getWidth(); w++)
        {
            for (int h = 0; h < reference.getHeight(); h++)
            {
    ...
    /**
     * Tests what happens if the unmasked gaps in the maskImage are smaller then the structuring element. (The
     * structuring element is used to close the image. If it can fit into a gap completely, the gap won't be closed.
     * It's default width and height are a tenth of the maskImage). <br>
     * The maskImage is painted using the blocks from markingX/ markingY, which makes a gap that fits the structuring
     * element exactly difficult.
     * 
     * @throws IOException
     */
    @Test
    public void correctlyClosedLess() throws IOException
    {
        ...
        Assert.assertTrue(compare.isEqual(reference, screenshot, fileMask, fileOut, differenceFile));
    }

    /**
     * Tests what happens if the unmasked gaps in the maskImage are bigger then the structuring element. (The
     * structuring element is used to close the image. If it can fit into a gap completely, the gap won't be closed.
     * It's default width and height are a tenth of the maskImage). <br>
     * The maskImage is painted using the blocks from markingX/ markingY, which makes a gap that fits the structuring
     * element exactly difficult.
    ...
    public void correctlyNotClosedMore() throws IOException
    {
        ...
        Assert.assertFalse(compare.isEqual(reference, screenshot, fileMask, fileOut, differenceFile));
    }

    /**
     * Tests if a gap in the corner will be closed if it is smaller then the structuring element. It should be.
     * 
     * @throws IOException
     */
    @Test
    ...
    /**
     * Tests if a gap in the corner will be closed if it is bigger then the structuring element. It shoudn't be.
     * 
     * @throws IOException
     */
    @Test
    ...
    /**
     * Tests what happens if a single pixel is different. Expected result: The marking block of that pixel is masked,
    ...
    /**
     * Tests what happens if there is a difference on every corner of the structure element. Calculation: MarkingX/ -Y =
     * 10 -> Every block of ten pixels corresponds to one pixel while closing. With an image width of 200 pixels, the
     * image to close will have a width of 20 pixels and the structure element a width of 3 pixels. Analogous for the
     * height. That means one difference in 40-50, none in 50-60, one in 60-70 None in the row below that. And again the
     * same in the row below that.
     * 
     * @throws IOException
     */
    @Test
