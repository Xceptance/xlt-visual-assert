...
/**
 * Class for comparison of images, written to continuously compare screenshots of websites. The class consists of a
 * wrapper method isEqual and several internal methods. It uses the ImageOperations class. The algorithm to be used,
 * fuzzyness parameters and whether or not a mask should be created is determined at the instantiation. The isEqual
 * method marks differences with rectangles. Their size can not be changed.
 * 
...
public class ImageComparison
{
    ...
    /**
     * The parameters pixelPerBlockXY and colTolerance define the fuzzyness of the comparison, higher parameters mean a
     * comparison that is less strict. The pixelPerBlock and pixTolerance parameters are only used in the 'FUZZY'
     * algorithm, the colTolerance in the 'FUZZY' and the 'PIXELFUZZY' algorithm.
     * <p>
     * In case images should only be equal in certain areas or certain areas should not be compared, a mask image can be
     * created. Black areas in the mask image will be ignored. Users can manually paint it black or use the trainingMode
     * to do so.
     * <p>
     * If the trainingMode parameter is true, differences will not be marked, instead the corresponding areas in the
     * mask image will be painted black. If the closeMask parameter is true, small gaps in he maskImage will be closed.
     * Black areas in the mask image will not be detected in later comparisons.
     * <p>
     * 
     * @param algorithm
     *            the algorithm the comparison should use. If the given string does not match any algorithm, it throws
     *            an IllegalArgumentException.
     * @param markingX
     *            determines the height of the blocks used for marking and masking. Has to be above 0
     * @param markingY
     *            determines the width of the blocks used for marking and masking. Has to be above 0.
     * @param fuzzyBlockDimension
     *            in the fuzzyEqual method, the images are divided into blocks for further fuzzyness. This parameter
     *            determines the width and height of the blocks.
     * @param colorTolerance
     *            up to where small differences in color should be tolerated. A value between zero and one should be
     *            given. Zero: No tolerance.
     * @param pixelTolerance
     *            how many differences per block will be tolerated, in percent. value between zero and one should be
     *            given. One: 100%. All pixels can be different.
     * @param trainingsMode
     *            whether or not the training mode should be used. If true, differences will be masked for later tests,
     *            not marked. And the comparison will always return true.
     * @param closeMask
     *            decides if small gaps in the mask should be closed after a training run.
     ...
     *            decides if a difference image should be created in addition to the markedImage. The difference image
     *            is a greyscale image, the higher the difference between the compared images, the lighter the
     *            corresponding area in the difference image.
     */
    public ImageComparison(final Algorithm algorithm, final int markingX, final int markingY, final int fuzzyBlockDimension,
                           final double colorTolerance, final double pixelTolerance, final boolean trainingsMode, final boolean closeMask,
                           final int closeMaskWidth, final int closeMaskHeight, final boolean differenceImage)
    {
    ...
    /**
     * Wrapper method for the different comparison methods. Handles resizing, masking, calls the comparison method and
     * marks the differences or masks them if trainingMode is true. Also draws the differenceImage if asked for. And
     * saves the marked image/ the mask image.
     * 
     * @param baselineImage
     *            the reference image
     * @param compareImage
     ...
     * @param fileMask
     *            the file where the mask image can be found or where it should be created
     * @param fileMarked
     *            the file where the marked image should be saved if there are differences
     * @param fileDifference
     *            the file where the difference image should be saved if the instance variable difference image is true
     * @return false if there were changes, true otherwise
     * @throws IOException
     */
    public boolean isEqual(final BufferedImage baselineImage, final BufferedImage compareImage, final File fileMask, final File fileMarked,
                           final File fileDifference) throws IOException
    {
     ...
     *         differences
     */
    private int[][] exactCompare(final BufferedImage img1, final BufferedImage img2) throws IOException
    {
    ...
    /**
     * Draws the differences onto the difference image with the correct grayness. Calculates the difference where there
     * was one and paints the pixel to the right color. Works directly on the difference image.
     * 
    ...
    /**
     * Initializes the mask Image. If there already is a mask Image in the file, it will simply read that. If not, it
     * will create a white Image of the same height and width as the picture given.
     * 
     * @param img
